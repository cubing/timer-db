...
const attemptIDRegex = /a_[0-9]{14}_[0-9a-f]{8}/;
...
const sessionIDRegex = /s_[0-9a-f]{8}/;

function isValidSessionUUID(id: string): boolean {
  return sessionIDRegex.test(id);
}

export function newSessionUUID(): SessionUUID {
  const suffix = bufferToHex(crypto.getRandomValues(new Uint8Array(8)));
